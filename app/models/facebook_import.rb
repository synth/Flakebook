class FacebookImport
  @queue = :file_serve
  def self.perform(access_token, flickr_username, photoset_id_string, skip_duplicates = false)
    ...
    sets.each do |set|
      client_manager.transfer_flickr_set_to_facebook_album(set, :skip_duplicates => skip_duplicates)
    end
